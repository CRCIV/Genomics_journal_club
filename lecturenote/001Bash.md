...
간략하게 파일을 테이블이라고 생각하면, 테이블의 특정 필드 (열)을 잘라내는 명령어.
...
텍스트 프로세싱에 특화된 스크립트로, 파일에서 원하는 부분을 찾아서 추출하기위해 사용하며, 매우 강력하면서 단순함.

```bash
awk 'BEGIN{<작업 수행전 파라미터들>} {<긱종 작업들.보통 print>} END {<모든 작업 끝나고 수행> }` 

```

* 내장 변수들
  - $1, $2, $3, $4, .... 각각의 column에 대한 변수. 
  - $0 : 라인 전체  
  - `NR` 파일의 라인 넘버
  - `NF` 파일의 필드 숫자.
